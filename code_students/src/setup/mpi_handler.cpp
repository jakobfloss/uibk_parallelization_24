...
mpi_handler::mpi_handler(const std::vector<int> &_num_tasks) {
	...
	// y-direction TBD by students - done
...
grid_3D mpi_handler::make_local_grid(const grid_3D &global_grid) {
	...
	// TBD by students - done
	bound_up_global[0] = global_grid.y_grid.get_left(num_cells_global[0]);
	bound_up_global[1] = global_grid.y_grid.get_left(num_cells_global[1]);
	bound_up_global[2] = global_grid.y_grid.get_left(num_cells_global[2]);
	...
	// To be repaired by students. - done
